...
// Pure parsing and serialization for spectralfix.ini. No Windows or Direct3D
// dependencies, so the whole settings round trip is unit testable.
...
namespace spectralfix
{
    ...
    // Accepts decimal and 0x-prefixed values. Rejects negatives, which std::stoul
    // would otherwise silently wrap into a large positive number.
    ...
    // Client-build-specific identity of the allocation SpectralFix enlarges.
    ...
    // User-facing appearance settings, valid independently of the selector.
    struct VisualSettings
    {
        float spreadOverride{kDefaultSpread}; // 0 means automatic.
    ...
    // Parses the whole file. Unknown keys are ignored without comment; known keys
    // carrying unusable values leave the existing value alone and add a warning.
